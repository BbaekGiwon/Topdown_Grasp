#!/usr/bin/env python3
"""
One-shot robot executor for Grasp_fruit top-down grasp pipeline.

Reads topdown_summary.json produced by run_topdown_grasp.py, then:
  STEP 1 : current → approach pose  (target Z + approach_offset)   [plan → RViz 확인 → 실행]
  STEP 2 : approach → target pose   (수직 하강)                      [plan → RViz 확인 → 실행]
  STEP 3 : hand 점진적 파지          (10 waypoint × 0.2 s)
  STEP 4 : lift                     (approach pose로 복귀)            [plan → RViz 확인 → 실행]

각 arm 이동 전 RViz에 경로가 시각화되고 터미널에서 y/n으로 확인 후 실행합니다.
...
Usage (must be run with ROS2 Humble sourced):
    ...
    python3 scripts/robot_executor.py \\
        ...
        [--execute_mode trajectory_forwarder] \\
...
# ── 팔/손 설정 — configs/arm.yaml, configs/hand.yaml 에서 관리 ────────────────
# PLANNING_GROUP, EE_LINK, REF_FRAME, PLANNING_TIME  → utils.arm
# HOME_JOINT_NAMES, HOME_JOINT_VALUES                 → utils.arm
# HAND_INIT_ENC, HAND_RELEASE_ENC, HAND_STEPS/PERIOD  → utils.hand
...
def _rotmat_to_quat(R):
    """3x3 rotation matrix → xyzw quaternion (numpy, Shepperd method)."""
...
def world_to_base(T_world_base_list, xyz_world, quat_world_xyzw):
    ...
    T_world_base: 4×4 list (world←base, i.e. p_world = T @ p_base).
...
class GraspExecutor(Node):
    ...
    def __init__(self, summary: dict, execute_mode: str, speed_factor: float,
        ...
        self._success             = False
        self._current_joints      = None   # /franka/joint_position 실시간 수신
        self._approach_traj       = None   # step_approach 궤적 → _finalize 역재생용
    ...
    def _setup_ros(self):
        ...
        self._cart_client = self.create_client(GetCartesianPath,
                                               ...
                                               callback_group=self._cb_group)
        self._ik_client   = self.create_client(GetPositionIK,
                                               '/compute_ik',
                                               callback_group=self._cb_group)
        ...
        # trajectory_forwarder 모드는 FollowJointTrajectory action 클라이언트도 갖되,
        # 단순 single-target arm 이동은 direct 토픽을 쓴다 (오케스트레이터 hand 시퀀스 우회).
        ...
        # Trajectory publisher — sends full JointTrajectory to SHM bridge
    ...
    # FR3 joint velocity limits (rad/s) — Franka Emika 사양
    _JOINT_VEL_LIMITS = [2.62, 2.62, 2.62, 2.62, 5.26, 4.18, 5.26]

    def _add_timestamps(self, jt: JointTrajectory) -> JointTrajectory:
        """Cartesian trajectory의 time_from_start 를 joint velocity limit 기반으로 채움.
        EXP_PC 가 times[i] <= times[i-1] 인 trajectory 를 거부하기 때문에 필수.
        """
        pts = list(jt.points)
        if len(pts) < 2:
            return jt
        ...
        for i in range(1, len(pts)):
            ...
            t += max(dt, 1e-3)   # 최소 1 ms per segment
            sec  = int(t)
            nsec = int((t - sec) * 1e9)
        ...
        self.get_logger().info(
            f'[TIMESTAMP] {len(pts)} pts  total={t:.2f}s')
        return result
    ...
    def _plan_ompl(self, goal: PoseStamped, label: str):
        ...
        if not self._wait(future, timeout):
            self.get_logger().error(f'[{label}] OMPL goal timeout')
            return None
        gh = future.result()
        if not gh.accepted:
            self.get_logger().error(f'[{label}] OMPL goal rejected')
            return None
        rf = gh.get_result_async()
        if not self._wait(rf, timeout):
            self.get_logger().error(f'[{label}] OMPL result timeout')
            return None
        res = rf.result().result
        if res.error_code.val != 1:
            self.get_logger().error(
                f'[{label}] OMPL failed  code={res.error_code.val}')
            return None
    ...
    def _plan_step(self, goal: PoseStamped, label: str,
        ...
        """
        계획 → RViz 표시 → y/n 확인 → JointTrajectory 반환 (취소/실패 시 None).

        PRIMARY   : Cartesian path (직선 EE 경로), max_step 단계적 완화 4회
        SECONDARY : seeded IK → joint-space OMPL (collision-checked 자유 경로)
        FALLBACK  : seeded IK → direct PtoP (최후 수단)
        """
        ...
        for max_step in [0.01, 0.02, 0.05, 0.1]:
            ...
            if cart is not None:
                ...
                # fraction < 1.0 : 나머지 거리를 seeded IK + PtoP 로 보정
                completion_jt = None
                ...
                if completion_jt is not None:
                    # partial Cartesian → 보정 PtoP 를 묶어서 반환할 수 없으므로
                    # 두 번 실행: Cartesian 먼저 보내고 즉시 completion 전송
                    ...
                    return completion_jt   # caller가 exec + wait 처리
                return jt
            print(f'  fraction 부족 — 재시도 (max_step 완화)...')

        # ── SECONDARY: seeded IK → joint-space OMPL (자유 경로) ─────────
        ...
        if seed is not None:
            ...
            if joints_target is not None:
                ...
                # ── FALLBACK: seeded IK → direct PtoP ────────────────────
    ...
    def _plan_joints(self, joint_names, joint_values, label: str):
        """관절 공간 목표로 OMPL 계획. start state를 _current_joints 로 명시."""
        g = MoveGroup.Goal()
        g.request.group_name                      = PLANNING_GROUP
        ...
        # is_diff=True 는 /joint_states 에 의존 → invalid state 오류 가능.
        # _current_joints 가 있으면 직접 명시해 MoveIt start-state 검증을 통과시킴.
        ...
        timeout = PLANNING_TIME + 4.0
        if not self._wait(future, timeout):
            self.get_logger().error(f'[{label}] goal timeout')
            return None
        gh = future.result()
        if not gh.accepted:
    ...
    def _compute_ik_seeded(self, goal: PoseStamped, seed_joints: list,
                           label: str) -> list | None:
        """seed_joints를 초기값으로 /compute_ik 호출 → 7-DOF joint 값 반환."""
        ...
        if not self._wait(future, 5.0):
            self.get_logger().error(f'[{label}] IK timeout')
            return None
        res = future.result()
        if res.error_code.val != 1:
    ...
    def _compute_fk(self, joint_values: list,
        ...
        """FK: joint values → base frame EE pose [x,y,z, qx,qy,qz,qw]."""
    ...
    def _plan_step_ik(self, goal: PoseStamped, label: str,
        ...
        """
        Seeded IK → confirm → joint 값 리스트 반환.
        IK 실패 시 Cartesian/OMPL fallback → 마지막 joint 추출.
        """
        ...
        jt = self._plan_step(goal, label,
                             jnames=jnames_seed, jvals=seed)
        if jt is None:
            return None
        return list(jt.points[-1].positions)

    def _exec_joints(self, joint_values: list) -> None:
        """joint 값 리스트를 /franka/target_joint 로 직접 전송."""
        spd      = Float64()
    ...
    def _wait_for_traj(self, dur: float, margin: float = 0.3) -> None:
        """trajectory duration 만큼만 대기 (EXP PC가 dur초 안에 완료 보장)."""
    ...
    def _wait_for_motion(self, target_joints: list,
        ...
        """joint 수렴 polling (duration 불명 시 fallback용)."""
        time.sleep(0.1)
        t0 = time.time()
    ...
    def _hold_hand_position(self, duration: float = 2.0) -> None:
        """비상정지 시 손을 현재 위치로 고정 (열지 않음).

        컨트롤러 타임아웃 전까지 현재 encoder 값을 반복 발행.
        _run_guarded()가 rclpy.shutdown() 호출 후 main() finally에서도
        호출될 수 있으므로 rclpy.ok() 확인 후 진행.
        Args:
            duration: 발행을 유지할 시간 (초)
        """
        if not rclpy.ok():
            return  # _run_guarded()가 이미 shutdown → publish 불가, 정상 종료
        enc = self._last_hand_enc
        if enc is None:
            return  # 손을 한 번도 움직이지 않은 경우 → 이미 초기 위치
        msg = Int16MultiArray()
        ...
        self.get_logger().info(
            f'[HOLD] 손 현재 위치 유지 ({duration:.1f}s)...')
    ...
    def _make_joint_traj(self, target: list, start: list = None) -> JointTrajectory:
        """MoveGroup 없이 2-point JointTrajectory 직접 생성 (fallback용).
        duration은 최대 관절 변위와 speed_factor 기반으로 보수적으로 산정.
        """
        if start is None:
    ...
    def _do_release_hand(self):
        """손 → HAND_RELEASE_ENC (손가락 펴짐) 로 점진적 이동 (확인 없음)."""
    ...
    def _finalize(self):
        ...
        if self._success:
            ...
            print('  ⚠  실행이 중단되었습니다.')
        # approach_traj는 팔이 approach 위치에 있을 때만 유효.
        # descent 이후 취소됐을 땐 arm이 target에 있으므로 OMPL 사용.
    ...
    def _exec_arm(self, jt):
        """JointTrajectory 실행: 전체 trajectory를 한 번에 전송 (smooth)."""
        self._exec_traj_smooth(jt)

    def _exec_traj_smooth(self, jt: JointTrajectory) -> list:
        """전체 JointTrajectory를 /franka/target_trajectory 로 publish.
        C++ bridge가 SHM에 적재 → libfranka joint position control로 추적.
        Returns: final joint positions (list[7]) or [].
        """
        if not jt.points:
            self.get_logger().error('[TRAJ] Empty trajectory')
            return []

        # joint_names 보정: 없으면 fr3_joint1..7 채움
        msg = JointTrajectory()
        msg.joint_names = list(jt.joint_names) if jt.joint_names else list(HOME_JOINT_NAMES)
        msg.points = jt.points

        # speed_factor를 SHM에 미리 설정 (PtoP 모드 호환)
        spd = Float64()
        spd.data = max(0.001, min(1.0, float(self._speed)))
        self._franka_speed_pub.publish(spd)
        time.sleep(0.02)
    ...
    def _exec_traj(self, jt):
        """FollowJointTrajectory 액션으로 전송 (planning_orchestrator 사용 시)."""
        goal = FollowJointTrajectory.Goal()
        goal.trajectory = jt
        future = self._traj_client.send_goal_async(goal)
        if not self._wait(future, 5.0):
            self.get_logger().error('Trajectory send timeout')
            return
        gh = future.result()
        if not gh.accepted:
    ...
    def _exec_waypoints(self, jt):
        """MoveIt JointTrajectory 를 waypoint 단위로 /franka/target_joint 에 publish.
        각 waypoint의 time_from_start 기반으로 타이밍을 맞춘다.
        """
        if not jt.points:
    ...
    def _exec_direct(self, jt):
        ...
        if len(final.positions) != 7:
            self.get_logger().error(
                f'Expected 7 joint positions, got {len(final.positions)}')
            return
        spd      = Float64()
        spd.data = max(0.001, min(1.0, float(self._speed)))
        self._franka_speed_pub.publish(spd)
    ...
    def _exec_hand(self, target_enc: list):
        """현재 손 위치에서 목표 encoder값까지 점진적으로 파지."""
        ...
        for i in range(1, HAND_STEPS + 1):
            ...
            self._hand_pub.publish(msg)
            self.get_logger().info(
                f'  [{i:2d}/{HAND_STEPS}] t={t:.1f}  raw={interp}')
            if i < HAND_STEPS:
                time.sleep(HAND_PERIOD)
    ...
    def _execute(self):
        ...
        bp     = grasp['base_pose']
        xyz_w  = bp['xyz']          # world frame
        quat_w = bp['quat_xyzw']    # world frame
        enc    = grasp['joint_angles_enc']
        T_wb   = self._summary.get('T_world_base')

        # world → base frame (MoveIt planning frame = URDF root 'base')
        if T_wb is not None:
            xyz_b, quat_b = world_to_base(T_wb, xyz_w, quat_w)
            # approach: world Z+offset → transform approach point too
            xyz_w_app  = [xyz_w[0], xyz_w[1], xyz_w[2] + self._approach_offset]
            xyz_b_app, quat_b_app = world_to_base(T_wb, xyz_w_app, quat_w)
        else:
            self.get_logger().warning(
                'T_world_base not in summary — using xyz as-is in base frame')
...
def parse_args():
    ...
    p.add_argument('--speed_factor', type=float, default=0.1,
                   help='direct_franka_topic 모드의 속도 (default: 0.1)')
    p.add_argument('--approach_offset', type=float, default=0.10,
                   help='Approach 자세의 Z 오프셋 m (default: 0.10)')
    return p.parse_args()
...
def main():
    ...
    try:
        ...
        node._hold_hand_position(duration=2.0)   # 비상정지 시 손 현재 위치 유지
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
